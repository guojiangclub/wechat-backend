;(function($) {
  ...
  $.extend(Plugin.prototype, {
    ...
    createPicker: function() {
      ...
      this.$picker.find('section')
        .height(parseInt(this.settings.height)); // 40 is height of the tabs
    ...
    updatePosition: function() {
      ...
      switch(this.settings.position) {
        case 'top':
          ...
          this.$picker.css({'top': 18-top + 'px', 'left':left + this.$el.outerWidth() - this.settings.width-60 + 'px'});
          break;
        case 'bottom':
          this.$picker.css({'left':'10','bottom':'240px'});
          break;
        case 'left':
          var left = this.$icon.width() + 10;
          this.$picker.css({'top':'-10px', 'right': left + 'px'});
          break;
        case 'right':
          var right = parseInt(this.settings.width) + this.$icon.width() - 30;
          this.$picker.css({'top':'-10px', 'right': -right + 'px'});
          break;
      }
    ...
    emojiClicked: function(e) {
      ...
      var emojiUnicode = findEmoji(emojiShortcode).desc;
        if(insertAtCaret)
  ...
  function insertAtCaret(inputField, myValue){
    if(document.all && inputField.createTextRange && inputField.caretPos){
      var caretPos=inputField.caretPos;
      caretPos.text = caretPos.text.charAt(caretPos.text.length-1) == '' ?
      myValue+'' : myValue;
    } else if(inputField.setSelectionRange){
      var rangeStart=inputField.selectionStart;
      var rangeEnd=inputField.selectionEnd;
      var tempStr1=inputField.value.substring(0,rangeStart);
      var tempStr2=inputField.value.substring(rangeEnd);
      inputField.value=tempStr1+myValue+tempStr2;
      //inputField.value=tempStr2+myValue+tempStr1;
      inputField.focus();
      var len=myValue.length;
      inputField.setSelectionRange(rangeStart+len,rangeStart+len);
      inputField.blur();
    }else{
