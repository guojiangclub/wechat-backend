...
class MaterialController extends Controller
{
    ...
    public function index(){
        ...
        if(empty(request('type'))||request('type')==self::TYPE_IMAGE){
            $type='image';
        }elseif (request('type')==self::TYPE_ViIDE){
            $type='video';
        }elseif (request('type')==self::TYPE_TEXT){
            $type='text';
        }elseif (request('type')==self::TYPE_ARTICLE){
            $type='article';
        }
...
//                  return $article->news_item;
    ...
    public function  storeText(Request $request){
        ...
        public function materialApi(){
            ...
            if (!empty(request('keyword'))&&$type==='text') {
                $key=request('keyword');
                $where['content'] = ['like', '%' . urldecode($key) . '%'];
            }
            if (!empty(request('keyword'))&&$type==='video') {
                $where['title'] = ['like', '%' . request('keyword') . '%'];
            }
            if (!empty(request('keyword'))&&$type==='article') {
                $where['title'] = ['like', '%' . request('keyword') . '%'];
            }
